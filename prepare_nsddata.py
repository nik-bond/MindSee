...
#     print(i)
# np.save('processed_data/subj{:02d}/nsd_train_stim_list{}.npy'.format(sub,sub),np.array(stim_indices) )
...
for i,idx in enumerate(test_im_idx):
    test_stim_indices.append(idx)
#     # stim_array[i] = stim[idx]
#     fmri_array[i] = fmri[sorted(sig_test[idx])].mean(0)
    print(i)

np.save('processed_data/subj{:02d}/nsd_test_stim_list{}.npy'.format(sub,sub),np.array(test_stim_indices) )
# np.save('processed_data/subj{:02d}/nsd_test_fmriavg_nsdgeneral_sub{}.npy'.format(sub,sub),fmri_array )
# # np.save('processed_data/subj{:02d}/nsd_tes
# t_stim_sub{}.npy'.format(sub,sub),stim_array )
